class AudioManager {
    ...
    startPlaying(arr, loop) {
        ...
        //initalizing players
        if (audioType === 'music') {
            this.$players.find('.music').on($.jPlayer.event.play, function () {
                if (this.playCallback) {
                    var mp3url = $(this).data('jPlayer').status.src;
                    var relativeUrl = decodeURIComponent(mp3url).slice(-arr[current].length);

                    if (relativeUrl === arr[current]) {
                        ID3.loadTags(mp3url, () => {
                            var tags = ID3.getAllTags(mp3url);
                            this.playCallback(tags);
                        });
                    }
                }
            });
