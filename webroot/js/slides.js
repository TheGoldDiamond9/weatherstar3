...
var travelForecastLength = 63000;
var currentProgram;
var currentDiv;
var goOn = true
	...
	function showSlides() {
		var slidePrograms = {
            ...
            currentConditions() {
                ...
                $('.info-cc .current-temperature').text('TEMP:  ' + weatherData.currentConditions.temp + '°F');
                if (weatherData.currentConditions.feelslike != weatherData.currentConditions.temp) {
					$('.info-cc .current-windchill').text(weatherData.currentConditions.feelsliketype + weatherData.currentConditions.feelslike + '°F');
				} else {
					$('.info-cc .current-windchill').text('');
				};
                $('.info-cc .current-humidity').text('HUMIDITY:  ' + weatherData.currentConditions.humidity + '%');
                $('.info-cc .current-dewpoint').text('DEWPOINT: ' + weatherData.currentConditions.dewpoint + '°F');
                $('.info-cc .current-pressure').text('BAROMETRIC PRESSURE: ' + weatherData.currentConditions.pressure + ' ' + weatherData.currentConditions.pressureTrend);
                $('.info-cc .current-wind').text('WIND: ' + weatherData.currentConditions.wind);
                $('.info-cc .current-visibility2').text(weatherData.currentConditions.visibility + ' MI.');
                if (weatherData.currentConditions.ceiling != null) {
					$('.info-cc .current-ceiling').text('CEILING: ' + weatherData.currentConditions.ceiling +' FT.')
				} else {
					$('.info-cc .current-ceiling').text('CEILING:UNLIMITED')
				};
            ...
            dayPartForecast() {
                ...
                $('#slide-title-text').text('YOUR NWS FORECAST -- ZONE ' + weatherData.NWS.zone);
                if (alerttext == "") {
				...
				dtimeOne()
                setTimeout(() => {dtimeTwo(), $('#slide-title-text').text('NAT\'L WEATHER SERVICE FORECAST')}, slideLength);
            ...
            extendedForecast() {
                ...
                for (let i = 0; i < 3; i++) {
                    let l = i+1
                    $('.extended-forecast .days .day-'+l).text(weatherData.extendedForecast.days[i].dayname);
                    $('.extended-forecast .extended-conditions .con-'+l).text(weatherData.extendedForecast.days[i].cond.replace("/", ", "))
                    $('.extended-forecast .extended-los .lo-'+l).text('LO:  ' + weatherData.extendedForecast.days[i].low);
                    $('.extended-forecast .extended-his .hi-'+l).text('HI:  ' + weatherData.extendedForecast.days[i].high);
                }
            ...
            ,travelForecast() {
				...
				for (var i = 0; i < weatherData.travel.cities.length; i++) {
					...
					$('#travel-container #travel-content .cities').text(prevTextCity += weatherData.travel.cities[i].displayname)
					let prevTextWeathers = $('#travel-container #travel-content .weathers').text()
					$('#travel-container #travel-content .weathers').text(prevTextWeathers += condReplace(weatherData.travel.cities[i].days[0].condition) + "\n")
					let prevTextHigh = $('#travel-container #travel-content .his').text()
					$('#travel-container #travel-content .his').text(prevTextHigh += weatherData.travel.cities[i].days[0].high + "\n")
					let prevTextLow = $('#travel-container #travel-content .lows').text()
					$('#travel-container #travel-content .lows').text(prevTextLow += weatherData.travel.cities[i].days[0].low + "\n")
				}
            ...
            ,almanac() {
                ...
                $('.almanac .header .header-1').text(weatherData.almanac.today);
                $('.almanac .header .header-2').text(weatherData.almanac.tomorrow);
