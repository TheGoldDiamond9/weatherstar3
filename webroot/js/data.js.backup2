    ...
    nwsClient.getCurrentConditions(locationConfig.mainCity.lat, locationConfig.mainCity.lon)
      .then(function(data) {
        try {
          ...
          weatherData.currentConditions.pressure = data.pressure || ""
          ...
          weatherData.currentConditions.dewpoint = data.dewpoint || ""
      ...
      const promise = nwsClient.getCurrentConditions(cityData.lat, cityData.lon)
        .then(function(data) {
          const nearbyCitiesObj = {
            ...
            condition: data.wxPhraseShort ? data.wxPhraseShort.replaceAll(' Shower', ' SHWR').replaceAll('Near', '').replaceAll('Light', 'LGT').replaceAll('Heavy', 'HVY').replaceAll('Early', 'ERLY').replaceAll('Partial', 'PART').replaceAll('Cldy', 'CLOUDY').replaceAll(' T-Storm', ' T-STM') : "",
        ...
        .then(function(forecastData) {
          ...
          if (periods && periods.length > 0) {
            ...
            const nearbyCitiesObjF = {
              ...
              condition: todayPeriod.shortForecast ? todayPeriod.shortForecast.replaceAll(' Shower', ' SHWR').replaceAll('Near', '').replaceAll('Light', 'LGT').replaceAll('Heavy', 'HVY').replaceAll('Early', 'ERLY').replaceAll('Partial', 'PART').replaceAll('Cldy', 'CLOUDY').replaceAll(' T-Storm', ' T-STM') : ""
            };
      ...
      const promise = nwsClient.getForecast(city.lat, city.lon)
        .then(function(forecastData) {
          ...
          if (periods && periods.length > 0) {
            ...
            weatherData.travel.cities[i].days[0].condition = todayPeriod.shortForecast ? todayPeriod.shortForecast.replaceAll(' Shower', ' SHWR').replaceAll('Near', '').replaceAll('Light', 'LGT').replaceAll('Heavy', 'HVY').replaceAll('Early', 'ERLY').replaceAll('Partial', 'PART').replaceAll('Cldy', 'CLOUDY').replaceAll(' T-Storm', ' T-STM') : ""
